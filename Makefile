...
HTTPS_GIT := github.com/cosnorton/cosnorton.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=cosnorton \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=cosnortond \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/cosnortond ./cmd/cosnortond
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/cosnortond.exe ./cmd/cosnortond
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/cosnortond
...
draw-deps:
	...
	@goviz -i ./cmd/cosnortond -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c cosnorton --local -f chains.yaml
endif
...
set-testnet-configs:
	cosnortond config set client chain-id chainid-1
	cosnortond config set client keyring-backend test
	cosnortond config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | cosnortond keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | cosnortond keys add acc1 --recover`
