...
# then log output of file /code/bin/cosnortond
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/cosnortond \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/cosnortond | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/cosnortond /usr/bin/cosnortond
...
CMD ["/usr/bin/cosnortond", "version"]
